// ============================
// script.js - Updated Version (with Live "Pling" Effect)
// ============================
...
function generateMultiLevelHeader(seriesHeaders) {
  ...
  while (col < headerParts.length) {
    const currentTop = headerParts[col][0];
    let colSpan = 1;
    while (col + colSpan < headerParts.length && headerParts[col + colSpan][0] === currentTop) {
      colSpan++;
    }
    row1.push({ content: currentTop, colSpan: colSpan });
    let subCol = col;
    while (subCol < col + colSpan) {
      const currentMid = headerParts[subCol][1];
      let subColSpan = 1;
      while (subCol + subColSpan < col + colSpan && headerParts[subCol + subColSpan][1] === currentMid) {
        subColSpan++;
      }
      row2.push({ content: currentMid, colSpan: subColSpan });
...
let rawSeriesData = []; // raw data from server
let lastSeriesData = []; // processed data for display/chart
let chart = null;
...
// Global objects for auto-refresh query control:
let queryStartTimes = {};
let queryMultipliers = {};
let globalEndMultiplier = 1; // multiplier for extending end time if no new data
...
// ---------- Helper: Apply Tag Settings to Raw Data ----------
function applyTagSettings(seriesData, tagSettings) {
  return seriesData.map(series => {
    return {
      name: series.name,
      data: series.data.map(pt => {
        let rawVal = pt[1];
        const errVal = tagSettings.error_values && tagSettings.error_values[series.name];
        if (errVal !== undefined && errVal !== null && parseFloat(rawVal) === parseFloat(errVal)) {
          rawVal = 0;
        }
        const scale = tagSettings.scale_factors && tagSettings.scale_factors[series.name] ? parseFloat(tagSettings.scale_factors[series.name]) : 1;
        let newVal = rawVal * scale;
        const maxDec = tagSettings.max_decimal && tagSettings.max_decimal[series.name] ? parseInt(tagSettings.max_decimal[series.name], 10) : null;
        if (maxDec !== null && !isNaN(maxDec)) {
          newVal = parseFloat(newVal.toFixed(maxDec));
        }
        return [pt[0], newVal];
      })
    };
...
function buildOneGroup(list) {
  ...
  list.forEach(item => {
    ...
    groups[group].push({ full: item.Tag, display: display });
  });
  return groups;
}
function buildTwoGroup(list) {
  ...
  list.forEach(item => {
    ...
    const display = parts.length > 2 ? parts.slice(2).join(".") : (sub ? sub : main);
    if (!mainGroups[main]) mainGroups[main] = {};
    if (!(sub in mainGroups[main])) mainGroups[main][sub] = [];
    mainGroups[main][sub].push({ full: item.Tag, display: display });
  });
  return mainGroups;
}
function buildTreeFromDisplay(list) {
  ...
  container.innerHTML = "";
  if (groupingMode === "0") {
    const flatTags = buildFlatTree(list);
    const ul = document.createElement("ul");
    flatTags.forEach(item => {
      const li = document.createElement("li");
      li.textContent = item.Tag;
      if (selectedTags.has(item.Tag)) li.classList.add("selected");
    ...
    logStatus(`Tag list updated in flat mode with ${flatTags.length} tags.`);
    ...
    groupKeys.forEach(group => {
      ...
      toggleBtn.addEventListener("click", e => {
        ...
        if (allSelected) groups[group].forEach(item => selectedTags.delete(item.full));
        else groups[group].forEach(item => selectedTags.add(item.full));
        buildTreeFromDisplay(list);
      });
      groupLi.appendChild(toggleBtn);
      ul.appendChild(groupLi);
      const subUl = document.createElement("ul");
      const items = groups[group].slice().sort((a, b) => a.display.localeCompare(b.display));
      if (sortOrder === "desc") items.reverse();
      items.forEach(item => {
        const li = document.createElement("li");
        li.textContent = item.display;
        if (selectedTags.has(item.full)) li.classList.add("selected");
        li.addEventListener("click", e => {
          e.stopPropagation();
    ...
    const mainKeys = Object.keys(mainGroups).sort();
    if (sortOrder === "desc") mainKeys.reverse();
    const ul = document.createElement("ul");
    mainKeys.forEach(main => {
      const mainLi = document.createElement("li");
      mainLi.textContent = main;
      ul.appendChild(mainLi);
      const subKeys = Object.keys(mainGroups[main]).sort((a, b) => {
        if (a === null || a === "null") return 1;
        if (b === null || b === "null") return -1;
        return a.localeCompare(b);
      });
      if (sortOrder === "desc") subKeys.reverse();
      subKeys.forEach(sub => {
        if (sub !== null && sub !== "null") {
          ...
          toggleBtn.addEventListener("click", e => {
            ...
            if (allSelected) mainGroups[main][sub].forEach(item => selectedTags.delete(item.full));
            else mainGroups[main][sub].forEach(item => selectedTags.add(item.full));
            buildTreeFromDisplay(list);
          });
          ...
          const items = mainGroups[main][sub].slice().sort((a, b) => a.display.localeCompare(b.display));
          if (sortOrder === "desc") items.reverse();
          items.forEach(item => {
            const li = document.createElement("li");
            li.textContent = item.display;
    ...
    container.appendChild(ul);
    logStatus(`Tag list updated in two-group mode with ${mainKeys.length} main categories.`);
  }
...
function updateChartBackground() {
  if (chart) {
    ...
    chart.update({
      ...
      xAxis: { labels: { style: { color: textColor } }, lineColor: textColor, tickColor: textColor },
...
document.getElementById("saveTagOptionsBtn").addEventListener("click", () => {
  ...
  for (let row of rows) {
    ...
    if (!isNaN(scaleVal)) {
      scaleVal = (scaleVal % 1 !== 0) ? parseFloat(scaleVal.toFixed(2)) : scaleVal;
    } else {
      scaleVal = 1.0;
    }
    if (errorVal !== null && !isNaN(errorVal)) {
      errorVal = (errorVal % 1 !== 0) ? parseFloat(errorVal.toFixed(2)) : errorVal;
    }
    newSettings.scale_factors[tag] = scaleVal;
    newSettings.error_values[tag] = errorVal;
    newSettings.max_decimal[tag] = (!isNaN(maxDec)) ? maxDec : 2;
  }
  ...
  if (rawSeriesData.length > 0) {
    ...
    lastSeriesData = fillSeriesData(applyTagSettings(deepClone(rawSeriesData), tagSettings));
...
async function onGraph(isAuto = false) {
  ...
  // Only extend end time if auto-refresh is active.
    ...
    const seriesData = results.map(result => {
      ...
      if (isAuto) {
        if (dataPoints.length > 0) {
          ...
          queryMultipliers[result.tag] = 1;
          queryStartTimes[result.tag] = Math.floor(lastTimestamp / 1000) + pollingRateSec;
        } else {
          queryMultipliers[result.tag] = (queryMultipliers[result.tag] || 1) + 1;
          queryStartTimes[result.tag] = result.qs + queryMultipliers[result.tag] * pollingRateSec;
        }
      }
      return { name: result.tag, data: dataPoints };
    });
    // Update globalEndMultiplier based on new data:
    if (isAuto) {
      if (totalNewData > 0) {
        globalEndMultiplier = 1;
      } else {
        globalEndMultiplier++;
      }
    }
    
    if (isAuto && rawSeriesData.length > 0 && chart) {
      seriesData.forEach((newSeries, i) => {
        if (!rawSeriesData[i]) {
          rawSeriesData[i] = newSeries;
        } else {
          newSeries.data.forEach(pt => {
            const lastRaw = rawSeriesData[i].data[rawSeriesData[i].data.length - 1];
            if (!lastRaw || lastRaw[0] < pt[0]) {
              rawSeriesData[i].data.push(pt);
            }
          });
        }
      });
      lastSeriesData = fillSeriesData(applyTagSettings(deepClone(rawSeriesData), JSON.parse(localStorage.getItem("tagSettings") || '{"scale_factors":{},"error_values":{},"max_decimal":{}}')));
      renderDataTable(lastSeriesData);
      const currentExtremes = chart.xAxis[0].getExtremes();
      while (chart.series.length > lastSeriesData.length) {
        chart.series[chart.series.length - 1].remove(false);
      ...
      // Follow new data if zoom is at right edge:
      const globalMax = Math.max(...lastSeriesData.map(s => s.data.length ? s.data[s.data.length - 1][0] : 0));
      if (Math.abs(currentExtremes.max - globalMax) < 2000) {
      ...
      lastSeriesData = fillSeriesData(applyTagSettings(rawSeriesData, JSON.parse(localStorage.getItem("tagSettings") || '{"scale_factors":{},"error_values":{},"max_decimal":{}}')));
      const appDark = document.body.classList.contains("dark-mode");
      const chartBg = appDark ? "#2e2e2e" : "#ffffff";
      const textColor = appDark ? "#e0e0e0" : "#000";
      const legendColor = appDark ? "white" : "#000";
...
// ---------- Render Data Table in Web App (with Multi-Level Header) ----------
...
// ---------- Merge Series Data into 2D Array ----------
...
// ---------- Excel Export (No Alignment, save as XLSX) ----------
...
async function onGenerateReport() {
  try {
    ...
    loadLogo(function (logoDataUrl) {
      if (logoDataUrl) {
        doc.addImage(logoDataUrl, "PNG", margin, 10, 60, 25);
      }
      doc.setFontSize(18);
      ...
      doc.addImage(chartImgData, "PNG", x, y, drawWidth, drawHeight);
      
      // ---- Data Table Page (Page 3) ----
      doc.addPage();
      doc.setFontSize(16);
